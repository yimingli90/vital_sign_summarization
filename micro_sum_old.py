...
def _get_micro_df():
    ...
    df_filtered = df[
        ...
        ~df['ResultFull'].str.contains('ERROR RPT', na=False) &
        ~df['Result'].str.contains('ERROR RPT', na=False) &
        ~(micro_df["ResultFull"].str.contains("NLS", case=False, na=False) |
          micro_df["ResultFull"].str.contains("NSSM", case=False, na=False) |
          micro_df["ResultFull"].str.contains("RSTM", case=False, na=False) |
          micro_df["ResultFull"].str.contains("USS", case=False, na=False) |
          micro_df["ResultFull"].str.contains("WIP", case=False, na=False) |
          micro_df["ResultFull"].str.contains("SLER", case=False, na=False) |
          micro_df["ResultFull"].str.contains("NT", case=False, na=False) |
          micro_df["ResultFull"].str.contains("NSR", case=False, na=False) 
          ) &
        ~((micro_df["ResultFull"].str.contains("NOT TESTED", case=False, na=False) |
        micro_df["ResultFull"].str.contains("NOT RECEIVED", case=False, na=False)) &
        micro_df['BugCode'].isna())
...
def _gen_final_results(cases):
    ...
    def sort_data(data: str):  
        ...
        for line in data.strip().split('\n'):
            if line[:2].isdigit() and line[2] == '/':  # 检查是否以日期开头（如 "09/05/17"）
        ...
        if current_entry is not None:  # 添加最后一条记录
            entries.append(current_entry)
        
        # 2. 解析日期并排序
        def extract_date(entry):
            date_str = entry.split(' – ')[0]  # 提取 "dd/mm/yy"
            return datetime.strptime(date_str, "%d/%m/%y")
        
        # 按日期降序（近到远）排序
        sorted_entries = sorted(entries, key=extract_date, reverse=True)
    ...
    for case_ in cases:
        ...
        for example in case_:
            ...
            micro_sum = micro_sum.replace('FAECES MOLECULAR ASSAY', 'FAECES MOLECULAR ASSAY PCR')
            example["micro_info"]['micro_sum'] = micro_sum
...
filtered_df = micro_df[
    ...
    micro_df["BatTestName"].str.contains("SCREEN", case=False, na=False)
  #  micro_df["BatTestName"].str.contains("COMMENT", case=False, na=False)
]
filtered_df = micro_df[
    #~micro_df["TestName"].str.contains("RESULT", case=False, na=False) &
     micro_df["ResultFull"].str.contains("NOT detected", case=False, na=False)
    # micro_df["BatTestName"].str.contains("SCREEN", case=False, na=False)
  #  micro_df["BatTestName"].str.contains("COMMENT", case=False, na=False)
]
...
filtered_df = micro_df[
                (micro_df["BatTestCode"].str.contains("HBCT", case=False, na=False) |
                  micro_df["BatTestCode"].str.contains("HBEB", case=False, na=False) |
                  micro_df["BatTestCode"].str.contains("HBAT", case=False, na=False) |
                  micro_df["BatTestCode"].str.contains("HBSG", case=False, na=False) |
                  micro_df["BatTestCode"].str.contains("HIVG", case=False, na=False) |
                  micro_df["BatTestCode"].str.contains("HCVA", case=False, na=False) 
                  )
]
#                 (df["BatTestName"].str.contains("MRSA", case=False, na=False) |
#                  df["BatTestName"].str.contains("ESBL", case=False, na=False) |
