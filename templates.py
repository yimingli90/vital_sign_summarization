...
TEMPLATE_TEST = """
##Task:
You are a doctor, please summarize the following patient febrile record concisely and clearly. For the febrile record, please focusing only on febrile and afebrile status and time duration. 

##Instructions:
Use only the provided information and time information. Do not make up any information that is not from the records. Please rcalculate the febrile duration and afebrile duration carefulle with the following Rules for Febrile Record. Summariza the vital sign records in 2 sentences. 
...
4. Fever duration calculation:
    • Starting from the last recorded fever before the cut-off time, trace backwards:
        • If another fever record (>37.7°C) is found within the 24 hours prior, extend the fever start time backwards to this record.
        • Continue until either:
            • A 24-hour gap without fever is encountered, or
            • The admission time is reached.
